...
class EnhancedDatasetExplorer:
    ...
    def load_dataset_cached(dataset_name, config, split):
    ...
    def get_dataset_names(_self):
        ...
        try:
            ...
            with st.spinner("Loading dataset list..."):
                # Fetch all datasets at once with a larger limit
                datasets = api.list_datasets(limit=None)
                
                # Extract dataset IDs
                all_datasets = [ds.id for ds in datasets]
                
                # Show completion
                st.sidebar.success(f"Loaded {len(all_datasets)} datasets")
            ...
            with st.spinner(f"Loading dataset '{dataset_name}'..."):
                ...
                if configs:
                    selected_config = st.selectbox(
                        ...
                        options=configs,
                        key=f"config_selector_{dataset_name}"  # Unique key for each dataset
                    )
                    
                    if selected_config:
                        try:
                            dataset = self.load_dataset_cached(dataset_name, selected_config, split)
                            return dataset, None
                        except ValueError as e:
                            if "trust_remote_code=True" in str(e):
                                return self._handle_remote_code(dataset_name, selected_config, split)
                            try:
                                dataset = self.load_dataset_cached(dataset_name, name=selected_config, split=split)
                                return dataset, None
                            except:
                                try:
                                    dataset = self.load_dataset_cached(dataset_name, configuration=selected_config, split=split)
                                    return dataset, None
                ...
                dataset = self.load_dataset_cached(dataset_name, split=split)
                return dataset, None
